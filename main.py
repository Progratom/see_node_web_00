...
def process():
  ...
  output_line = tharkun_bot(input_line)
  #print(output_line)
  if type(input_line) != str:
    input_line = ""
  return render_template('index.html', input=input_line, output=output_line)
...
def tharkun_bot(inpt):
  ...
  if inpt == "help":
    ...
    return random.choice(data["facts"])
      ...
      "rhyme"
  ):  #form "rhyme:"your_language":"your_word, returns list of minimum 10 words with same ending (as many letters as possible)
    if len(inpt.split(":")) == 3:
      language = inpt.split(":")[1]
      word = inpt.split(":")[2]
      arr1 = data["rhyme"][
        language]  #"smaller" list. Just words that fits. In the begining bigger of the two, because lists are switsched on beggining of algorithm
